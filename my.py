...
class KnowledgeBaseApp:
    ...
    def end_classification(self, classification_group):
        ...
        for rule in self.rules:
            if rule["current animal group"] == classification_group and "end classification" in rule:
                ...
                # Iterate through animal_media to find the correct group
                ...
                if rule_found:
                    break  # Exit the loop once the rule is found and the images are displayed

        # Hide buttons after classification
    ...
    def display_animal_images(self, group_name, classification_message):
        # Iterate through the animal_media to find the correct group
        for group in self.animal_media:
            if group_name in group:
                media = group[group_name]  # Access the group correctly
                message_frame = tk.Frame(self.main_frame, bg='#A5D6A7')
                message_frame.place(relx=0.5, rely=0.45, anchor="center")
                parts = classification_message.split("\n")
                main_message = parts[0]
                ...
                for animal in media["Images"]:
                    ...
                    x_pos += 220
                break  # Break after finding the group to avoid unnecessary iterations
