...
"""
HAS SUBCATEGORIES INCLUDED + SHOWS THE DECISION PROCESS
"""
...
class KnowledgeBaseApp:
    ...
    def display_next_question(self, question_number):
        print(f"Searching for question number: {question_number}")
        for rule in self.rules:
            # Split the rule's question number into parts and check if it matches the given number
            rule_numbers = rule["question number"].split(" and ")
            print(f"Checking rule with question number: {rule['question number']}")
            if question_number in rule_numbers or rule["question number"] == question_number:
                print(f"Match found for question number: {rule['question number']}")
                self.current_question = rule
                self.current_feature_index = 0  # Reset feature index for the new rule
                self.ask_feature_question()
                return
        print("No matching rule found for question number.")
    ...
    def answer(self, user_input):
        ...
        print(f"Answer recorded: {feature_name} = {self.answers[feature_name]}")
    ...
    def determine_next_step(self):
        ...
        print(f"Evaluating rule: {current_rule}")
        print(f"Collected answers: {self.answers}")
        ...
        if len(features) == 1:  # Single feature question
            ...
            print(f"Decision: all_true={all_true}, all_false={all_false}, next_step={next_step}")

        # Check if `next_step` corresponds to a classification or another question
        if next_step in [group["animal group"] for group in self.knowledge_base]:
            self.end_classification(next_step)
        else:
            self.display_next_question(next_step)


    def find_rule_number(self, animal_group):
        for rule in self.rules:
            if rule.get("current animal group") == animal_group:
                return rule["question number"]
        return None
